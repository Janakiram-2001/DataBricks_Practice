...
@dp.materialized_view(
    name="logistics_dp.logistics_schema.gold_staff_geo_enriched_dlt2",
    comment="Staff enriched with Geo Location data",
    table_properties={"quality": "gold"})
def gold_staff_geo_enriched_dlt2():
    # Read the upstream silver tables
    df_staff = spark.read.table("logistics_dp.logistics_schema.silver_staff_dlt3")
    df_geo = spark.read.table("logistics_dp.logistics_schema.silver_geotag_dlt3")
...
@dp.materialized_view(
    name="logistics_dp.logistics_schema.gold_shipment_stats1",
    comment="Aggregated Shipment statistics by Source City",
    table_properties={"quality": "gold"}
)
def gold_shipment_stats2():
    return (
        spark.read.table("logistics_dp.logistics_schema.silver_shipments_dlt3")
        .groupBy("source_city")
        .agg(
